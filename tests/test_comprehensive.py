...
class TestLLMProvider:
    ...
    def test_get_llm_no_providers_raises(self):
        ...
        get_llm.cache_clear()
        with (
            patch("dailyai.llm.provider._build_providers", return_value=[]),
            pytest.raises(RuntimeError, match="No LLM providers"),
        ):
            get_llm()
    ...
    async def test_warmup_hf_with_token_handles_error(self):
        ...
        with (
            patch.dict(os.environ, {"HF_API_TOKEN": "test-token"}, clear=False),
            patch("dailyai.llm.provider.httpx") as mock_httpx,
        ):
...
class TestCollectorNode:
    ...
    async def test_collector_runs_with_mocked_rss(self):
        ...
        with patch(
            "dailyai.graph.nodes.collector._fetch_rss_feed",
...
class TestNewsService:
    ...
    async def test_get_feed_normalizes_country(self):
        ...
        with patch("dailyai.services.news.db") as mock_db:
            ...
            mock_db.get_articles = AsyncMock(
                return_value=[
                    {
                        "title": f"Article {i}",
                        "summary": "s",
