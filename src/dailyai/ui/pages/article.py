...
async def article_page(article_id: str):
    ...
    if params:
        ...
        if article is None:
            ...
            for field in (
                'headline', 'summary', 'why_it_matters', 'topic',
                'source_name', 'source_trust', 'sentiment',
                'article_url', 'published_at',
            ):
    ...
    with ui.column().classes('w-full max-w-3xl mx-auto min-h-screen pb-24 sm:pb-28 px-4 pt-6'):
        with ui.row().classes('w-full items-center justify-between mb-4'):
            ...
            ui.button('Open Source', on_click=lambda: ui.navigate.to(source_url, new_tab=True)).props('flat color=accent icon=open_in_new')

        if not article:
            with ui.column().classes('w-full items-center justify-center py-20 opacity-70'):
                ui.icon('error_outline', size='52px').classes('mb-4').style('color: var(--text-muted)')
                ui.label('Article not found. It may have expired from the current feed.').classes('text-secondary text-center')
        else:
            with ui.card().classes('w-full glass-card p-6'):
                ui.label(article.get('topic', 'Top Stories')).classes('text-[11px] font-black tracking-wider text-accent mb-2')
                ui.label(article.get('headline', '')).classes('text-2xl md:text-3xl font-black leading-tight mb-3')
                ui.label(article.get('summary', '')).classes('text-[15px] text-secondary leading-relaxed mb-4')

                why = article.get('why_it_matters', '').strip()
                if why:
                    with ui.column().classes('w-full bg-elevated/50 p-4 rounded border border-light mb-5'):
                        ui.label('Why it matters').classes('text-[12px] font-bold text-accent uppercase tracking-wider mb-1')
                        ui.label(why).classes('text-sm text-primary italic')

                with ui.row().classes('w-full justify-between text-[12px] text-muted gap-4 flex-wrap'):
